...
# Number of schools reached = £100 million/ £20,000 = 5,000 schools in England
# Number of schools in England = 24,025 [2]
# Percentage of schools reached = 20.8% =  20% (rounding down as a conservative estimate)
# Therefore, we assume that we are able to reach 20% of the children in 5 - 17 years in England


# Eligibility criteria:
# Children in age group [5 - 17] years
# Number of children aged 5 - 17 years in England = 8,723,931 [1]
# Number of children reached = 20% of 8,723,931 = 1,744,786
...
df = read.csv(here("inputs/processed/hse_2019_children.csv")) %>%
  mutate(eligibility_criteria = case_when(age >= 5 & age <= 17 ~ 1,
                                          TRUE ~ 0))

# Eligibility criteria:
# Children in age group 5 - 12 years
# Number of children in age group 5 - 12 years = 5,597,481 [1]
# Therefore, 25% children in this age group = 25% x 5,597,481 = 1,399,370
...
child_bar_plot = child_bmi_change %>%
  ...
  labs(fill = "", 
       ...
       y = "Prevalence - %",
       subtitle = "Children - England | Policy 22") +
...
write_xlsx(path = "outputs/policy_22/policy_22_england.xlsx", x = table_outputs)


# full datafile outputs with intervention assignment
write.csv(intervention_df, file = "outputs/policy_22/policy_22_child_england_bmi.csv")
