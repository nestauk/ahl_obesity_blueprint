...
# test = intervention_df %>%
#   select(intervention_year1, intervention_year2) %>%
#   summarise(counts = sum(wt_int),
#             count_1 = n()) %>%
#   mutate(percent_share = (counts/sum(counts))*100) %>%
#   mutate(number_people = counts*44263393)
...
ggsave(here("outputs/policy_25/policy_25_updated_impact_England_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')
...
write.csv(post_df_adult, file = "outputs/policy_25/policy_25_updated_adult_england_bmi.csv")


# bmi year on year prevalence:
# write.csv(bmi_change_year, file = "outputs/policy_25/policy_25_updated_adult_england.csv")
write_xlsx(x = bmi_change_year, path = "outputs/policy_25/policy_25_updated_adult_england.xlsx")
