# Case 3: everyone over BMI 40 doesn't receive reduction:
...
df = df %>%  # processed dataset with variables such as height, weight, bmi, daily energy intake passed as input to the function 
  ...
  mutate(intake_diff = case_when(bmi_interest_classs == "overweight" ~ intake_change_ov,
                                 ...
                                 TRUE ~ intake_diff)) %>%
  mutate(intervention = ifelse(bmi_interest_classs %in% c("underweight", "normal", "morbidly obese" ), "No", "Yes"))
...
write_xlsx(post_df_adult, path = "C:/git/ahl_obesity_blueprint/outputs/ravi_hugo_query/case_3_summary.xlsx")
write_xlsx(bmi_change_year, path = "C:/git/ahl_obesity_blueprint/outputs/ravi_hugo_query/case_3_detailed.xlsx")
