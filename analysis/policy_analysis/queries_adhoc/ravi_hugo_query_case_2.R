# Case 2: everyone over BMI 35 doesn't receive reduction:
...
intake_change_ob_35_40 = 0
intake_change_mob = 0 # 64
...
df = df %>%  # processed dataset with variables such as height, weight, bmi, daily energy intake passed as input to the function 
  ...
  mutate(intake_diff = case_when(bmi_interest_classs == "overweight" ~ intake_change_ov,
                                 ...
                                 TRUE ~ intake_diff)) %>%
  mutate(intervention = ifelse(bmi_interest_classs %in% c("underweight", "normal", "obese_35_40", "morbidly obese" ), "No", "Yes"))
...
write_xlsx(post_df_adult, path = "C:/git/ahl_obesity_blueprint/outputs/ravi_hugo_query/case_2_summary.xlsx")
write_xlsx(bmi_change_year, path = "C:/git/ahl_obesity_blueprint/outputs/ravi_hugo_query/case_2_detailed.xlsx")
