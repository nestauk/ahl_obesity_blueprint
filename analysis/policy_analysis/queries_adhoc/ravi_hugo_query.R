# Case 1: everyone over BMI 25 receives reduction:
...
intake_change_ov = -78
intake_change_ob_30_35 = -78    # 54
intake_change_ob_35_40 = -78
intake_change_mob = -78 # 64
...
df = df %>%  # processed dataset with variables such as height, weight, bmi, daily energy intake passed as input to the function 
  ...
  mutate(bmi_interest_classs = case_when(bmi <= 18.5 ~ "underweight",
                                         bmi > 18.5 & bmi < 25 ~ "normal",
                                         bmi >= 25 & bmi < 30 ~ "overweight",
                                         bmi >= 30 & bmi < 35 ~ "obese_30_35",
                                         bmi >= 35 & bmi < 40 ~ "obese_35_40",
                                         bmi >= 40 ~ "morbidly obese",
                                         TRUE ~ "NA")) %>%
  mutate(intake_diff = ifelse(bmi_interest_classs %in% c("underweight", "normal"), 0, 99)) %>% # creating a variable to store the change in energy intake due to intervention and applying it only to those with BMI >= 25
  mutate(intake_diff = case_when(bmi_interest_classs == "overweight" ~ intake_change_ov,
                                 bmi_interest_classs == "obese_30_35" ~ intake_change_ob_30_35,
                                 bmi_interest_classs == "obese_35_40" ~ intake_change_ob_35_40,
                                 bmi_interest_classs == "morbidly obese" ~ intake_change_mob,
                                 TRUE ~ intake_diff)) %>%
  mutate(intervention = ifelse(bmi_interest_classs %in% c("underweight", "normal"), "No", "Yes"))
...
post_df_adult = post_df %>%
  select("id", "weight", "height", "age", "sex", "bmi", "wt_int", "psu", "strata", "pal",
         "rmr", "bmi_interest_classs", "intake", "intervention", "intake_diff", "1", "365", "730", "1095", "1460", "1825") %>% # 
  rename(bmi_0 = "1", bmi_1 = "365", bmi_2 = "730", bmi_3 = "1095", bmi_4 = "1460", bmi_5 = "1825") %>% 
  mutate(bmi_0_class = case_when(bmi_0 <= 18.5 ~ "underweight",
                                 bmi_0 > 18.5 & bmi_0 < 25 ~ "normal",
                                 bmi_0 >= 25 & bmi_0 < 30 ~ "overweight",
                                 bmi_0 >= 30 & bmi_0 < 35 ~ "obese_30_35",
                                 bmi_0 >= 35 & bmi_0 < 40 ~ "obese_35_40",
         ...
         bmi_1_class = case_when(bmi_1 <= 18.5 ~ "underweight",
                                 ...
                                 bmi_1 >= 30 & bmi_1 < 35 ~ "obese_30_35",
                                 bmi_1 >= 35 & bmi_1 < 40 ~ "obese_35_40",
         ...
         bmi_2_class = case_when(bmi_2 <= 18.5 ~ "underweight",
                                 ...
                                 bmi_2 >= 30 & bmi_2 < 35 ~ "obese_30_35",
                                 bmi_2 >= 35 & bmi_2 < 40 ~ "obese_35_40",
         ...
         bmi_3_class = case_when(bmi_3 <= 18.5 ~ "underweight",
                                 ...
                                 bmi_3 >= 30 & bmi_3 < 35 ~ "obese_30_35",
                                 bmi_3 >= 35 & bmi_3 < 40 ~ "obese_35_40",
         ...
         bmi_4_class = case_when(bmi_4 <= 18.5 ~ "underweight",
                                 ...
                                 bmi_4 >= 30 & bmi_4 < 35 ~ "obese_30_35",
                                 bmi_4 >= 35 & bmi_4 < 40 ~ "obese_35_40",
         ...
         bmi_5_class = case_when(bmi_5 <= 18.5 ~ "underweight",
                                 ...
                                 bmi_5 >= 30 & bmi_5 < 35 ~ "obese_30_35",
                                 bmi_5 >= 35 & bmi_5 < 40 ~ "obese_35_40",
...
bmi_change = rbind(
  ...
  post_df_adult %>% 
    count(bmi_interest_classs, wt = wt_int) %>% 
    mutate(freq = n/sum(n)*100,
           type = "Year 0") %>% 
    rename(BMI = bmi_interest_classs))

bmi_change = bmi_change %>%
  mutate(BMI = factor(BMI, levels = c("underweight", "normal", "overweight", "obese_30_35", "obese_35_40", "morbidly obese"))) %>%
  as.data.frame()
...
bmi_change_year = bmi_change %>%
  ...
  select(type, underweight, normal, overweight, obese_30_35, obese_35_40, `morbidly obese`)

bmi_change_year


write_xlsx(post_df_adult, path = "C:/git/ahl_obesity_blueprint/outputs/ravi_hugo_query/case_1_summary.xlsx")
write_xlsx(bmi_change_year, path = "C:/git/ahl_obesity_blueprint/outputs/ravi_hugo_query/case_1_detailed.xlsx")


mean_df = post_df_adult %>%
  group_by(bmi_interest_classs) %>%
  summarise(mean_BMI = weighted.mean(bmi_0, wt = wt_int, na.rm = TRUE))
