...
# (1) Adults 18+: Number of adults in age group 18+ years in Scotland = 4,434,138 [1]
# (2) Living in IMD areas 4 and 5 and in a food desert in Great Britain = 1,200,000 [2]
# (3) Living in IMD areas 1 and 2 and in a food desert in England & Wales = 1,106,301 [2]
# (4) Living in IMD areas 1 and 2 and in a food desert in Scotland = 1,200,000 - 1,106,301 = 93,699
...
# Modelling approach:
# (1) We first filter for those living in SIMD 1 or 2, then randomly select 1.2 million individuals.
...
# 1.2. Selecting adults living in SIMD 1 or 2

df = read.csv(here("inputs/processed/shes_2019.csv")) %>%
...
adult_bar_plot = bmi_change %>%
  ...
  labs(fill = "", 
       ...
       y = "Frequency",
       subtitle = "Population | Scotland") +
  theme_ipsum(base_size = 8, axis_title_size = 8) + #, base_family="Averta"
  theme(legend.position = "top")

adult_bar_plot

ggsave(here("outputs/policy_15/policy_15_impact_scotland_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')

write.csv(post_df_adult, file = "outputs/policy_15/policy_15_adult_scotland_bmi.csv")
write_xlsx(path = "outputs/policy_15/policy_15_Scotland.xlsx", x = table_outputs)
