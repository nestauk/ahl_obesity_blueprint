...
##############################################################################################
# Policy 19: Invest £50 million over 5 years in Local Authorities to plan and deliver        #
#            active transport                                                                #
#                                                                                            #
##############################################################################################

# Note: the budget allocation is taken on advice of policy spec team.
# £500 million is allocation for England, a proportional allocation on the basis of population is
# calculated and rounded up
# £500 million is allocation for 56,286,961 people, therefore, for 4,434,138 people the allocation
# would be £48.5 million, rounding this up to £50 million for Scotland
...
post_df_adult = post_df %>%
  select("id", "weight", "height", "age", "sex", "bmi", "wt_int", "psu", 
         "strata", "pal", "rmr", "bmi_class", "intake", "energy_expenditure", "1", "365", "730", "1095", "1460", "1825" ) %>%
...
adult_bar_plot = bmi_change %>%
  ...
  labs(fill = "", 
       ...
       y = "Frequency",
       subtitle = "Scotland - Policy 19") +
  theme_ipsum(base_size = 8, axis_title_size = 8) + #, base_family="Averta"
  theme(legend.position = "top")

adult_bar_plot

ggsave(here("outputs/policy_19/policy_19_impact_Scotland_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')
...
write_xlsx(path = "outputs/policy_19/policy_19_scotland.xlsx", x = table_outputs)

# Output 3: Cost Modelling input files:
write.csv(post_df_adult, file = "outputs/policy_19/policy_19_adult_scotland_bmi.csv")
