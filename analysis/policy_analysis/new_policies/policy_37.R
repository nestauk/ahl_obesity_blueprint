...
#   We use the number of people from the NHS 10 year plan = 250,000 additional
#   people offered the programme per year.
#   However, it is unlikely that everyone offered the programme will take up
#   treatment. We use stats from Taylor et al. (2024) (c) to estimate the
#   number of people who enroll and the share of people who experience weight
#   loss
...
# inputs:
NUMBER_OF_PEOPLE_OFFERED_DWMPS <- 250000
SHARE_EXPERIENCING_WEIGHT_LOSS <- 0.45
NUMBER_OF_PEOPLE_EXPERINCING_WEIGHT_LOSS <- NUMBER_OF_PEOPLE_OFFERED_DWMPS * SHARE_EXPERIENCING_WEIGHT_LOSS
ENGLAND_ADULT_POPULATION <- MODEL_CONSTANTS$ENGLAND_ADULT_POPULATION
...
#' FUNCTION: select_intervention_sample()
#' Selects a weighted random sample for an intervention over multiple years.
#'
#' This function iteratively selects a sample for an intervention across a specified
#' number of years. In each year, it targets individuals meeting specific criteria
#' and who have not received the intervention in previous years. The sampling is
#' performed using a weighted random selection process to achieve a total weight
#' representative of the desired sample size within the eligible population.
#' The logic for this function is explained here:
#' https://docs.google.com/document/d/1b8eo_wgedOrJ-D5AWqIjCxmCr_yu-ez3EA6uZkwMvmQ/edit?usp=sharing
#'
#' @param data A data frame containing the population data.
#' @param sample_size The desired constant sample size (number of individuals) for the 
#'                    intervention in *each* year.
#' @param population_size The total size of the overall population being modeled (e.g.,
#'                        if the data represents a sample of a larger population).
#' @param weight_var A string specifying the name of the column in data that 
#'                   contains the sampling weights (e.g., "wt_int").
#' @param num_years The total number of years the intervention will run for.
#' @param criteria_1 A string specifying the name of the column used for the primary
#'                   eligibility criteria (e.g., "eligibility).
#' @param criteria_1_value The specific value in \code{criteria_1} that defines eligibility
#'                         (e.g., eligibility indicator value).
#'
#' @return A data frame identical to the input \code{data}, but with \code{num_years}
#' new columns added (named 'intervention_year1', 'intervention_year2', etc.).
#' These columns are populated with "Yes" for individuals selected for the
#' intervention in that year, and "No" otherwise.
#'
#' @details
#' The function operates yearly:
#' \enumerate{
#'   \item Identifies the **eligible population** (those meeting \code{criteria_1} and not previously selected for the intervention).
#'   \item Calculates the total size of the eligible population based on its proportion of the total \code{weight_var} sum, scaled by \code{population_size}.
#'   \item Calculates the **desired weight sum** for the sample, which is proportional to the overall weight of the eligible subset, scaled down to represent the \code{sample_size} vs \code{eligible_population}.
#'   \item Performs weighted random sampling (without replacement from the eligible pool) until the cumulative sum of the \code{weight_var} for the selected individuals meets or exceeds the **desired weight sum**.
#'   \item Updates the intervention status for the current year and records the individuals in the \code{intervention_history} to exclude them from future years' sampling.
#' }
#'
#' @examples
#' \dontrun{
#' # Assuming 'my_data' has columns 'wt_int', 'bmi', and 'bmi_cat'
#' my_data_with_intervention <- select_intervention_sample(
#'   data = my_data,
#'   sample_size = 500,
...
select_intervention_sample <- function(data,
                                       sample_size,
                                       population_size,
                                       weight_var,
                                       num_years,
...
post_df_adult <- post_df_adult %>%
  mutate(
    bw_y1 = weight + weight_loss_y1 + weight_regain_y1,
  ...
  mutate(
    bmi_1_class = case_when(
      bmi_y1 <= 18.5 ~ "underweight",
    ...
    bmi_2_class = case_when(
      bmi_y2 <= 18.5 ~ "underweight",
    ...
    bmi_3_class = case_when(
      bmi_y3 <= 18.5 ~ "underweight",
    ...
    bmi_4_class = case_when(
      bmi_y4 <= 18.5 ~ "underweight",
    ...
    bmi_5_class = case_when(
      bmi_y5 <= 18.5 ~ "underweight",
