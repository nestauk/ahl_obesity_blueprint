...
################################################################################################### 
# Policy 3x_5 : Option 5: Behaviourial weight management programme                                #
#                                                                                                 #
###################################################################################################
...
# weight regain: (Option 5)
# They regain 30% of the lost weight in the second year and then weight remains constant
...
assign_weight_changes <- function(data, bodyweight_var, num_years, weight_loss_1, weight_loss_2, weight_regain) {
  ...
  for (year in 1:num_years) {
    ...
    browser()
    # weight regain to be assigned only from the year after intervention,
    # so in this case, it would be weight loss in year 1 and weight regain in the following year
    if (year > 1) {
      ...
      if (year == 2){
        ...
        prev_intervention_cols <- paste0("intervention_year", (year - 1))  
...
post_df_adult = assign_weight_changes(data = df, bodyweight_var = "weight",
                                          ...
                                          weight_loss_2 = 0, weight_regain = 0.35*2.4)
...
# Additional anaysis to estimate and check the correct number of people are chosen for and receiving the intervention:
...
write_xlsx(bmi_change_year, path = "outputs/new_policies/policy_3x_op1/bwms_5_adult_england.xlsx")
...
ggsave(here("outputs/new_policies/policy_3x_op1/bwms_5_impact_England_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')
...
write.csv(post_df_adult, file = "outputs/new_policies/policy_3x_op1/bwms_5_adult_england_bmi.csv")
