...
assign_weight_changes <- function(data, bodyweight_var, num_years, weight_loss_1, weight_loss_2, weight_regain) {
  ...
  for (year in 1:num_years) {
    ...
    if (year > 1) {
      ...
      prev_intervention_cols <- paste0("intervention_year", 1:(year - 1))
      prev_intervention <- apply(data[, prev_intervention_cols] == 1 | data[, prev_intervention_cols] == 2, 1, any)
...
post_df_adult = assign_weight_changes(data = df_selected, bodyweight_var = "weight",
                                ...
                                weight_loss_2 = 5.6, weight_regain = 0.01*12)
...
write.csv(bmi_change_year, file = "outputs/policy_6/policy_6_updated_adult_england.csv")
...
ggsave(here("outputs/policy_6/policy_6_updated_impact_England_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')
...
write.csv(post_df_adult, file = "outputs/policy_6/policy_6_updated_adult_england_bmi.csv")



result_df = read_csv(here("outputs/policy_6/policy_6_updated_adult_england_bmi.csv"))
