...
# (2) We assume that the £30 mil would be distributed to 3 local authorities at £10 mil each. We find that on average local authorities
#     (County, Unitary Authority, Boroughs and Metropolitan districts) together make up 22 units with an average population of 143,313 people.
#     Implementing the policy in 3 local authorities @ £10 mil per authority implies that ~ 429,939 individuals [6] would be exposed to the policy.
#     Further, given that the proportion of people living with excess weight is ~ 70% of the population of Wales, which equals 300,957 individuals
#     in these 3 local authorities 
#     Therefore, we choose ~12% of the population to receive this intervention
...
select_intervention_sample <- function(data_df, bmi_threshold, required_proportion, 
...
df = read_csv(here("inputs/processed/nsw_2019.csv"))

# Selecting individuals to receive intervention

set.seed(193)

number_of_la = 3
people_per_la = 114679
adults_wales = 2522940


total_people_la = people_per_la * number_of_la
percent_excess_weight = 0.695
total_eligible_la = percent_excess_weight * total_people_la
total_eligible_wales = percent_excess_weight * adults_wales # 0.03311258

proportion_to_select = total_eligible_la / total_eligible_wales
proportion_to_select_updated = proportion_to_select/2 # assuming that 50% of the people living with excess weight increase their physical activity

df_selected = select_intervention_sample(data_df = df,
                                         bmi_threshold = 25, # interrested in impact of policy on those living with excess weight
                                         required_proportion = proportion_to_select_updated, # 0.016, # 0.5, # assuming that 50% of those living with excess weight increase their physical activity
                                         weight_var = "wt_int",
                                         bmi_var = "bmi",
                                         num_years = 1)
...
post_df_adult = post_df %>%
  select("id", "weight", "height", "age", "sex", "bmi", "wt_int", "pal",
...
ggsave(here("outputs/policy_19/policy_19_impact_wales_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')
...
write_xlsx(path = "outputs/policy_19/policy_19_updated_wales.xlsx", x = table_outputs)

# Output 3: BMI files:
write.csv(post_df_adult, file = "outputs/policy_19/policy_19_updated_adult_wales_bmi.csv")
