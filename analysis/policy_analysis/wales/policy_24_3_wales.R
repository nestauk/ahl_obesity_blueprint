...
# Description: 100% of weight loss occurs in the first year on the drug, weight gain in third year then plateaus
...
# Number of people the funding can reach = £30 million/£3,360 = 8,928 individuals
# Population estimate for adults equal and over 18 years of age = 2,522,940 (a)
...
# (bmi >= 30) | (bmi >= 27.5 + ethnicity = Asian - Indian(9)
#                                          Asian - Pakistani (10), Asian - Bangladeshi (11), Asian - Chinese (12), 
#                                          Asian - Other (13), Black - African (14), Black - Caribbean (15)
#                                          Black - Other (16)
#                                       
...
df = df_wales_cleaned %>%
  mutate(eligibility = case_when(bmi >= 30 ~ 1,
                                 ...
                                 (bmi >= 27.5 & ethnicity %in% c(9, 10, 11, 12, 13, 14, 15, 16)) ~ 1,
                                 TRUE ~ 0))


set.seed(2493)

# Number of people the funding can reach = £30 million/£2,440 = 12,295 individuals
# Population estimate for adults equal and over 18 years of age = 2,522,940 (a)

df_updated = select_intervention_sample(data = df,
                                        sample_size = 8928,
...
post_df_adult = assign_weight_changes(data = df_updated,
                                      bodyweight_var = "weight",
                                      num_years = 5,
                                      weight_loss_percent = 0.111,
                                      weight_regain = 0.67)
...
ggsave(here("outputs/policy_24_3/policy_24_3_updated_impact_Wales_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')

write_xlsx(bmi_change_year, path = "outputs/policy_24_3/policy_24_3_updated_adult_wales.xlsx")

write.csv(post_df_adult, file = "outputs/policy_24_3/policy_24_3_updated_adult_wales_bmi.csv")
