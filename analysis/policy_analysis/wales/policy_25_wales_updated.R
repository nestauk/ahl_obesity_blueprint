...
# thresholds updated following EAG advice to move bmi cut off to 30:
...
# function to select the sample of people and assign intervention
...
select_intervention_sample <- function(data, bmi_threshold, sample_size, population_size, 
  ...
  for (year in 1:num_years) {
    ...
    while (current_weight_sum < desired_weight_sum) { # && abs(desired_weight_sum - current_weight_sum) < 0.01 && abs(desired_weight_sum - current_weight_sum) > 0 
...
# function to assign weight changes
...
# assigning intervention for the 7000 additional bariatric surgery
intervention_df = select_intervention_sample(data = df_wales_cleaned,
                                             bmi_threshold = 30,
                                             sample_size = 6500,
...
ggsave(here("outputs/policy_25/policy_25_updated_impact_Wales_Adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')

write_xlsx(x = bmi_change_year, path = "outputs/policy_25/policy_25_updated_adult_wales.xlsx")

write.csv(post_df_adult, file = "outputs/policy_25/policy_25_updated_adult_wales_bmi.csv")
