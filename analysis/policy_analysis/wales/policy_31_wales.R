...
################################################################################################### 
# Policy 31: Allocate £10 million per year to fund a programme of financial incentives to         #
...
# would be £9.7 million, rounding this up to £10 million for wales
...
# 2. Individual only receives the treatment once, i.e. if they receive the intervention in year 1,
#    then they are not eligible to receive it again in a subsequent year
# 3. Weight regain is 0.01 kg per month post the year of intervention - using the weight regain as per
...
# 2. Individuals living in areas of deprivation (SIMD 1 & 2)

# Estimating the number of people exposed to the policy:
# Budget allocated to the policy is £6 million per year, and @ £215 per year per individual
# Number of people exposed to policy = £6 million/ £215 = 27,907 individuals
# Adult population of Wales = 2,522,940 [1]
...
# cleaning raw Welsh Health Survey 2019 data file and reading in the cleaned csv
...
df = df %>%
  mutate(eligibility = case_when(bmi >= 25 & simd_updated == 1 ~ 1, TRUE ~ 0))
...
# Estimating the number of people exposed to the policy:
# Budget allocated to the policy is £10 million per year, and @ £215 per year per individual
# Number of people exposed to policy = £6 million/ £215 = 27,907 individuals
# Adult population of Wales = 2,522,940 [1]

df = select_intervention_sample(data = df,
                                sample_size = 27907,
                                population_size = 2522940,
                                weight_var = "wt_int",
                                bmi_var = "bmi",
                                num_years = 5,
...
adult_bar_plot = bmi_change %>%
  ...
  theme_ipsum() +
  labs(fill = "", 
       title = "Wales | Policy 31", 
       y = "Frequency",
       subtitle =  "BMI Categories Distribution") +
...
ggsave(here("outputs/policy_31/policy_31_impact_wales_adult.png"), 
       plot = adult_bar_plot, 
       width = 10, 
       height = 6,
       bg='#ffffff')
...
write.csv(post_df_adult, file = "outputs/policy_31/policy_31_adult_wales_bmi.csv")


# Output 3: Year wise BMI prevalence:
write_xlsx(path = "outputs/policy_31/policy_31_wales.xlsx", x = table_outputs)
