...
df_child2 <- read_csv(here("inputs/processed/hse_2019_children.csv")) %>%  # we are reading a file that contains energy intake, BMI weight class and other variables reated during the pre-processing stage.
...
energy_matrix2 = apply(df_child2, 1, generate_interpolated_energy)
...
child_model_weight2 = child_weight(age = df_child2$age,
                                  sex =  df_child2$sex, 
                                  #FM = df_child2$fm_hudda, 
                                  #FFM = df_child2$ffm_hudda, 
                                  EI = energy_matrix2,   
                                  days = (365*5),
                                  checkValues = TRUE)


child_bw_h2 = as.data.frame(child_model_weight2[["Body_Weight"]]) %>%
...
child_age_h2 = as.data.frame(child_model_weight2[["Age"]]) %>%
...
child_ffm_h2 = as.data.frame(child_model_weight2[["Fat_Free_Mass"]]) %>%
...
child_fm_h2 = as.data.frame(child_model_weight2[["Fat_Mass"]]) %>%
...
child_age_bw_ffm_fm2 = cbind(child_bw_h2, child_age_h2, child_ffm_h2, child_fm_h2) %>%
...
new_df_child2 = cbind(df_child2, child_age_bw_ffm_fm2) %>%
  rowwise() %>%
...
child_bmi_change_year2 = child_bmi_change2 %>%
  select(-c(n)) %>%
  pivot_wider(., names_from = BMI, values_from = freq)

write.csv(child_bmi_change_year2, file = "C:/git/ahl_obesity_blueprint/outputs/data/child_bmi_2.csv"  )
